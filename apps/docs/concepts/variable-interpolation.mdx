...
1. **Built-in functions** -- `$uuid()`, `$now()`, etc.
2. **Environment file** -- `.hitspec.env.json` values
3. **Config environments** -- `hitspec.yaml` `environments` section
4. **Inline variables** -- `@variable = value` in the file
5. **Captured values** -- `{{requestName.captureName}}`
