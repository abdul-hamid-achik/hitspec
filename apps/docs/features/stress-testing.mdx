...
The [API Client Manager](/guides/api-client-manager) stress test view lets you create, edit, and delete profiles without touching `hitspec.yaml` directly. Select a profile from the dropdown to populate the stress config fields, or click **New Profile** to save the current configuration. All changes are persisted to `hitspec.yaml` automatically.
