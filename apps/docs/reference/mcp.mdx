...
startup. MCP initialization instructions are built from that registered
surface, so clients are not prompted to use unconfigured search or capture
tools. They also state whether `hitspec_fetch` is public-only or has
